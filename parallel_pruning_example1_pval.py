...
df_rows.to_csv('new_data/synth1.csv')

#### #### #### #### #### #### #### ####

# 
# rows = pd.read_csv ('new_data/synth1.csv', sep='\t')
x_dim = len(rows[0])-1
...
scores_reps = Parallel(n_jobs=min(total_reps, 10))(delayed(OneRep)(rep_no) for rep_no in range(total_reps))   
