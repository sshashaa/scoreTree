...
s1 = np.random.lognormal(shape1, scale1, 1000)

shape2, scale2 = 1/3, 0.6
s2 = np.random.lognormal(shape2, scale2, 1000)

shape3, scale3 = 1/4, 0.3
s3 = np.random.lognormal(shape3, scale3, 1000)

shape4, scale4 = 1/5, 0.3
s4 = np.random.lognormal(shape4, scale4, 1000)
...
plt.savefig('synth_fig/synth7_hist.png')
plt.legend()
plt.show()
...
plt.savefig('synth_fig/synth7_test.png')
plt.show()
...
for metric_id, met in enumerate(metrics):
    for th_id, th in enumerate(prune_thr_list):
        ...
        if metric_id == 0:
            axes[metric_id, th_id].set_title('Threshold: ' + str(th))
plt.savefig('synth_fig/synth7_train.png')
plt.show()