...
shape1, scale1 = 1, 4.  
s1 = np.random.normal(shape1, scale1, 1000)
plt.hist(s1, density=True, alpha=0.5, label='N1', bins=50, color='blue')

shape2, scale2 = 1, 6 
s2 = np.random.normal(shape2, scale2, 1000)
plt.hist(s2, density=True, alpha=0.5, label='N2', bins=50, color='red')
plt.legend()
plt.show()

shape3, scale3 = -1., 4
s3 = np.random.normal(shape3, scale3, 1000)
plt.hist(s3, density=True, alpha=0.5, label='N3', bins=50, color='green')

shape4, scale4 = -1., 6 
s4 = np.random.normal(shape4, scale4, 1000)
plt.hist(s4, density=True, alpha=0.5, label='N4', bins=50, color='orange')
plt.legend()
plt.show()
...
def synthetic1(n):
    ...
    for id_x, x_i in enumerate(x):
        if x_i < 0:
            if x_i < -0.5:
                y = np.random.normal(shape1, scale1, 1)
            else:
                y = np.random.normal(shape2, scale2, 1)                    
        else:
            if x_i < 0.5:
                y = np.random.normal(shape3, scale3, 1)
            else:
