...
def accuracy_crpsnew(leaf_dict, args):  
    ...
    for key, val in leaf_dict.items(): # key is X and val is y
        ...
        total_crps += denum*sum([sum((leaf_ytrain - np.float64(y)) * (nlength * (leaf_ytrain > np.float64(y)) - idlist + 0.5) * leaf_ytrain_freq)*leaf_ytest_freq[j] for j, y in enumerate(leaf_ytest)])
